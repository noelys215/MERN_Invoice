...
export const PaymentHistory = ({ sortPaymentHistory }) => {
	const [page, setPage] = useState(0);
	const [rowsPerPage, setRowsPerPage] = useState(5);

	const rows = sortPaymentHistory;

	/* Avoid layout jumps when reaching the end page with empty rows. */
				...
				<TableBody>
					{(rowsPerPage > 0
						...
						<StyledTableRow
							...
							<StyledTableCell>{addCurrencyCommas(row?.amountPaid)}</StyledTableCell>
							<StyledTableCell>{row?.paymentMethod}</StyledTableCell>
							<StyledTableCell>
								{row?.additionalInfo ? row?.additionalInfo : 'No additional info'}
							</StyledTableCell>
						</StyledTableRow>
					))}
