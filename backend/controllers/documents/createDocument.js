...
const createDocument = asyncHandler(async (req, res) => {
	...
	if (!customer) {
		res.status(404);
		throw new Error('Customer does not exist for the currently logged in user');
	}

	if (customer.createdBy.toString() !== req.user._id.toString()) {
		res.status(400);
		throw new Error('Unauthorized to create documents for customers you did not create');
	}
