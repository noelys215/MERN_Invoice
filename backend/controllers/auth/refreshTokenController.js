...
// $-auth    Public
// Rotating the refresh tokens, deleting the old ones, creating new ones and detecting token reuse
...
const newAccessToken = asyncHandler(async (req, res) => {
	...
	jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET_KEY, async (err, decoded) => {
		...
		if (err || existingUser._id.toString() !== decoded.id) return res.sendStatus(403);
