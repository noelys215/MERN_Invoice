...
const resendEmailVerificationToken = asyncHandler(async (req, res) => {
	...
	if (user.isEmailVerified) {
		res.status(400);
		throw new Error('Account has already been verified; Please login');
	}

	let verificationToken = await VerificationToken.findOne({ _userId: user._id });

	if (verificationToken) await VerificationToken.deleteOne();

	const resentToken = randomBytes(32).toString('hex');

	let emailToken = await new VerificationToken({ _userId: user._id, token: resentToken }).save();
